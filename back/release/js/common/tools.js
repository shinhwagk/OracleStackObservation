...
export function executeNoLoginShellCommand(command) {
...
function commandCheck(currStatus, cci) {
    if (currStatus) {
...
function pingCheck(ip) { return executeNoLoginShellCommand(`ping -c 2 ${ip}`); }
function portCheck(ip, port) { return executeNoLoginShellCommand(`nc -v -w 4 ${ip} -z ${port}`); }
function execRemoteShellCommand(node, shellCommand) {
