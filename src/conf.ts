...
import { ConfNode, ConfDatabase, ConfAlertOracle, ConfAlertOS } from './objects';

export function getNodeConfFile(): Promise<ConfNode[]> {
  return readFile('./conf/nodes.json').then(JSON.parse)
}

export function getAlertConfFile() {
...
//   }
//   return flatten((await getNodeConfFile()).map(node => node.databases.map(db => genDatabaseConf(node, db))))
// }
...
//   return (await getNodeConfFile()).filter(d => d.ip === ip)[0]
// }